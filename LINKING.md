...
3. **Halaman callback FE per-integrasi**: `WEB_APP_URL/{integration}/link/callback`
   (mis. `/gworkspace/link/callback`, `/spotify/link/callback`). Backend
...
per-integrasi `WEB_APP_URL/gworkspace/link/callback`, dengan query param:

```
https://<web-app>/gworkspace/link/callback?code=4/0A...&state=gworkspace.1751...abc
```

Integrasi diketahui dari route (`/{integration}/link/callback`), bukan dari
isi `state`. Halaman mengambil `code` and `state` dari query param apa adanya
...
4. **Env wajib di-set** (fatal kalau kosong):
   - `WEB_APP_URL` — origin web app (mis. `https://chat.avagenc.com`).
     ...
     (`WEB_APP_URL/gworkspace/link/callback`, `.../spotify/link/callback`); tiap URL wajib
...
(`WEB_APP_URL/spotify/link/callback`) dengan `?code=&state=spotify.…` (atau
...
2. **Env baru wajib di-set** (fatal kalau kosong):
   - `SPOTIFY_CLIENT_ID` / `SPOTIFY_CLIENT_SECRET` — OAuth app di Spotify
     ...
     daftarkan `WEB_APP_URL/spotify/link/callback` verbatim di app dashboard
     Spotify.)
